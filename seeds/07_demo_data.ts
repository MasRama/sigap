...
export function run(SQLite: typeof SQLiteType): void {
  ...
  if (announcementCount === 0 && adminUser) {
    SQLite.exec`
      ...
      VALUES (${randomUUID()}, ${'Selamat Datang di SIGAP'}, ${'Sistem informasi sekolah sudah aktif. Guru dapat mengisi jurnal, verifikasi mengajar, dan input nilai. Orang tua dapat memantau nilai dan kehadiran anak.'}, ${adminUser.id}, ${now}, ${now})
    `;
  }
