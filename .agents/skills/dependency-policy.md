...
| QR code | `qrcode` (generate, server-side), `html5-qrcode` (scan, in-browser) | qr.js, qrcode-generator, external APIs | Standard npm packages, no external API calls; `html5-qrcode` handles camera lifecycle + decode + file scan, same as sibling project dani-ejournal |
